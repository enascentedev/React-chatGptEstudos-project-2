	...
	const gerarResposta = async () => {
		try {
			const response = await fetch('http://localhost:8080/chat', {
				...
				body: JSON.stringify({
					materia: materia, // Enviar a matéria selecionada
					descricao: assuntoSelecionado // Enviar o assunto selecionado
				})
			});
			if (!response.ok) {
	...
	return (
		<div className="bg-base-200 flex flex-col gap-10 p-10">
			...
			<div className="flex items-center gap-2 h-40 my-10">
				...
				<textarea className="input input-bordered h-40 w-full p-5">{resposta || "A resposta aparecerá aqui..."}</textarea>
			</div>
		</div>
	);
}
